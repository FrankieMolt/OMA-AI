                ...
                <Card className="bg-memoria-bg-card border-memoria-border-muted rounded-sm p-6">
                   ...
                   <div className="relative mb-6">
                      ...
                      <input 
                        ...
                        className="w-full pl-9 pr-4 py-3 bg-memoria-bg-ultra-dark border border-memoria-border-muted rounded-sm text-xs text-memoria-text-hero focus:outline-none focus:border-memoria-text-hero transition-all placeholder-memoria-text-whisper"
                      />
                   </div>
                   <div className="space-y-2 max-h-64 overflow-y-auto custom-scrollbar pr-2">
                      {filteredNodes.map(node => (
                         <button key={node.id} className="w-full flex items-center justify-between p-3 bg-memoria-bg-ultra-dark border border-memoria-border-muted rounded-sm group hover:border-memoria-text-hero transition-all cursor-pointer">
                            <div className="flex items-center gap-3">
                               <div className="w-1.5 h-1.5 rounded-full bg-memoria-text-whisper group-hover:bg-memoria-text-hero transition-colors" />
                               <span className="text-xs font-light text-memoria-text-secondary group-hover:text-memoria-text-hero transition-colors">{node.title}</span>
                            </div>
                            <span className="text-[9px] text-memoria-text-meta">{node.connections} links</span>
                         </button>
                      ))}
                   </div>
                   <Button className="w-full mt-6 bg-memoria-text-hero text-memoria-bg-ultra-dark hover:bg-memoria-text-secondary h-10 rounded-sm text-[10px] font-bold uppercase tracking-widest">
                      <Plus size={14} className="mr-2" /> Add Entity
                   </Button>
                </Card>

                <Card className="bg-memoria-bg-card border-memoria-border-muted rounded-sm p-6">
                   <h2 className="text-xs uppercase tracking-widest text-memoria-text-meta mb-4 font-normal">System Health</h2>
                   ...
                   <div className="w-full bg-memoria-bg-ultra-dark h-1 rounded-full overflow-hidden">
                      <div className="bg-memoria-text-hero h-full w-3/4" />
                   </div>
                ...
                <Card className="bg-memoria-bg-card border-memoria-border-muted rounded-sm h-[600px] relative overflow-hidden flex flex-col items-center justify-center p-10">
                   ...
                   >
                      <div className="absolute inset-0 flex items-center justify-center">
                         <div className="w-4 h-4 bg-memoria-text-hero rounded-full blur-md animate-pulse" />
                      </div>
                      {nodes.map((node, i) => {
                         ...
                         return (
                            ...
                            >
                               <div className="absolute top-4 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-all pointer-events-none bg-memoria-bg-surface border border-memoria-border-default text-memoria-text-hero px-2 py-1 rounded-sm text-[9px] font-bold uppercase whitespace-nowrap z-50">
                                  {node.title}
                               </div>
                            </motion.div>
                         );
                   ...
                   <div className="mt-20 text-center relative z-10">
                      <h3 className="text-2xl font-light font-display mb-4 text-memoria-text-hero">Neural Knowledge Mapping</h3>
                      <p className="text-sm text-memoria-text-whisper max-w-sm font-light leading-relaxed">
                         Your mental schema is being actively indexed. 
                         Navigate the nodes to explore connected concepts.
                      </p>
                      <div className="flex gap-4 mt-8 justify-center">
                         <Badge variant="outline" className="rounded-sm border-memoria-border-muted text-memoria-text-meta">2D Projection</Badge>
                         <Badge variant="outline" className="rounded-sm border-memoria-border-muted text-memoria-text-meta">Topological</Badge>
                      </div>
                   </div>
                   ...
                   <div className="absolute bottom-6 right-6">
                      <div className="p-3 bg-memoria-bg-ultra-dark border border-memoria-border-muted rounded-sm flex items-center gap-3">
                         ...
                         <span className="text-[10px] uppercase tracking-widest font-bold text-memoria-text-secondary">Real-time Sync Active</span>
                      </div>
                   </div>
                </Card>
             </div>
