...
## FRANKIE's AgentMail (Active)

**Email:** frankie@agentmail.to  
**Display Name:** Frankie  
...
```json
{
  "mcpServers": {
    "AgentMail": {
      ...
      "env": {
        "AGENTMAIL_API_KEY": "am_da2d2be6b9fe4aab80e7f27ff1e4f7a6326bd1fad14a94d0f32fb5f9b79406d2"
      }
    }
  }
}
...
### Available Tools
- `get_message` - Retrieve emails from inbox
- `send_message` - Send emails
- `reply_to_message` - Reply to existing emails
...
**Service:** https://agentmail.to  
**Documentation:** https://github.com/agentmail-to/agentmail-mcp
...
| **agentmail-mcp** | latest | Email inbox API for AI agents | ✅ Ready |

### MCP Servers (Active)
| Server | Type | Purpose | Status |
|--------|------|---------|--------|
| **agentmail-mcp** | Official | Email API for agents (send/receive) | ✅ Ready |
| **context7** | Docs | Documentation MCP | ✅ Ready |
| **mcporter** | Management | MCP server management | ✅ Ready |
...
```bash
# Send email via MCP
npx -y agentmail-mcp --tools send_message

# Check inbox
npx -y agentmail-mcp --tools get_message

# Reply to email
npx -y agentmail-mcp --tools reply_to_message
```
...
✅ **Email** - AgentMail inbox (frankie@agentmail.to)  
