...
export const metadata: Metadata = {
  ...
  description: 'View and manage all your resume analyses. Track scores, compare jobs, and optimize your profile for better matches.',
  alternates: {
...
export default function ResumeDashboardPage() {
  const stats = [
    ...
    { label: 'Analyses Completed', value: '0', icon: <CheckCircle size={20} /> },
      ...
      <section className="pt-48 pb-12 px-4 md:px-14">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-lg text-memoria-text-whisper max-w-2xl font-light">
             Track your resume analyses, view job match scores, and optimize your profile 
             for better opportunities. All data securely encrypted and private.
          </p>
        </div>
      </section>
      ...
      <section className="py-12 px-4 md:px-14">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
             {stats.map((stat, i) => (
                <Card key={i} className="bg-memoria-bg-card border border-memoria-border-muted p-6">
                   <CardContent className="p-0">
                      <div className="flex items-center gap-3 mb-2">
                         <div className="p-2 bg-memoria-bg-surface border border-memoria-border-default rounded">
                            {stat.icon}
                         </div>
                         <span className="text-[10px] text-memoria-text-meta uppercase tracking-widest">{stat.label}</span>
                      </div>
                      <div className="text-3xl font-black text-memoria-text-hero font-display">{stat.value}</div>
                   </CardContent>
                </Card>
             ))}
          </div>
      ...
      <section className="py-24 px-4 md:px-14 text-center">
        <div className="max-w-2xl mx-auto">
          ...
          <p className="text-base text-memoria-text-whisper mb-8 font-light">
             Upload your resume and let AI identify your strengths, find skill gaps, 
             and match you with the best opportunities. Takes less than 30 seconds.
          </p>
