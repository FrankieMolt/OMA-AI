...
import { Terminal, Zap, ShieldAlert, Activity, Database, ChevronRight, Play, Square } from 'lucide-react';
...
const LOGSOURCES: LogEntry[] = [
  { id: 'kernel', type: 'success', icon: Terminal, message: 'Kernel v6.14.3-OMA loaded successfully.' },
  { id: 'network', type: 'info', icon: Database, message: 'Synchronizing with Base-Mainnet (Chain ID: 8453)...' },
  { id: 'api', type: 'warning', icon: Zap, message: 'High API latency detected on Solana RPC (450ms). Retrying with fallback.' },
  { id: 'security', type: 'error', icon: ShieldAlert, message: 'Unverified agent signature detected from IP 192.168.1.42. Firewall engaged.' },
  { id: 'activity', type: 'info', icon: Activity, message: 'Agent #8821 completed DeepScrape job. +0.003 USDC earned.' },
  { id: 'wallet', type: 'success', icon: Square, message: 'AgentWallet adapter v2.4.2 connected. Balance: 0.42 USDC.' },
];
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold uppercase transition-all ${
              isLive ? 'bg-green-500/20 text-green-400 hover:bg-green-500/30' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700'
            }`}
          >
            ...
            {logs.map((log) => (
              ...
              >
                ...
                <div className="flex-1">
                  <div className={`flex items-center gap-2 mb-1 ${log.type === 'error' ? 'text-red-400' : log.type === 'warning' ? 'text-yellow-400' : log.type === 'success' ? 'text-green-400' : 'text-blue-400'}`}>
                    {React.createElement(log.icon, { size: 14 })}
                    <span className="truncate">{log.message}</span>
                  </div>
                  {log.type === 'error' && (
                    <span className="text-[10px] text-zinc-600 ml-2">[CRITICAL]</span>
                  )}
                </div>
              </motion.div>
            ))}
