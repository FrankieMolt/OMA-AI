...
export const metadata: Metadata = {
  ...
  description: 'Explore the powerful features of OMA-AI marketplace for AI agents.',
}
...
export default function FeaturesPage() {
  ...
  const colorClasses: Record<string, string> = {
    ...
    green: 'from-green-500/10 to-green-600/10 text-green-400 border-green-500/30',
    purple: 'from-purple-500/10 to-purple-600/10 text-purple-400 border-purple-500/30',
    orange: 'from-orange-500/10 to-orange-600/10 text-orange-400 border-orange-500/30',
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-[#050510]/90 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <a href="/" className="flex items-center gap-3 text-white cursor-pointer group">
            ...
            <span className="font-bold text-lg tracking-tight font-orbitron">Features</span>
          </a>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((feature, i) => (
              <div key={i} className={`p-8 rounded-2xl bg-gradient-to-br ${colorClasses[feature.color].split(' ').slice(0, 2).join(' ')} border ${colorClasses[feature.color].split(' ').slice(3).join(' ')} hover:scale-105 transition-all cursor-pointer group`}>
                <div className={`w-14 h-14 rounded-xl bg-gradient-to-br ${colorClasses[feature.color].split(' ').slice(0, 2).join(' ')} flex items-center justify-center mb-6 group-hover:scale-110 transition-transform`}>
                  {feature.icon}
                </div>
          ...
          <div className="mt-16 p-8 rounded-2xl bg-gradient-to-br from-blue-500/10 to-purple-500/10 border border-blue-500/20 text-center">
            ...
            <p className="text-gray-300 mb-8 max-w-2xl mx-auto">
              Start integrating AI agents with instant micropayments today. No credit card required.
            </p>
