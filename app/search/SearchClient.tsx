      ...
      <section className="pt-32 pb-12 px-4 md:px-14 border-b border-memoria-border-muted">
        <div className="max-w-7xl mx-auto">
          ...
          <h1 className="text-4xl md:text-7xl font-light tracking-tighter mb-8 font-display text-memoria-text-hero">
            Global
            <br />
            <span className="text-memoria-text-secondary">Search</span>
          </h1>
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {results.map((service) => (
                  <Card
                    key={service.id}
                    className="bg-memoria-bg-card border-memoria-border-muted rounded-sm p-8 hover:border-memoria-border-active transition-all group"
                  >
                    ...
                    <div className="flex items-center justify-between pt-6 border-t border-memoria-border-muted">
                      <div className="flex items-baseline gap-1">
                        <span className="text-lg font-light text-memoria-text-hero">
                          ${service.price_per_use || service.price}
                        </span>
                      ...
                      <Button
                        variant="outline"
                        className="border-memoria-border-muted text-memoria-text-whisper hover:text-white rounded-sm h-10 text-[10px] font-bold uppercase tracking-widest"
                      >
                        Details
                      </Button>
                    </div>
                ...
                {results.length === 0 && !loading && (
                  <div className="col-span-full text-center py-32 border border-dashed border-memoria-border-muted rounded-sm">
                    ...
                    <p className="text-memoria-text-whisper text-lg font-light">
                      No matches found in the current index.
                    </p>
                  </div>
                )}
              </div>
