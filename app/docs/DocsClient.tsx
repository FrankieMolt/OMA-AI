...
const docSections: DocSection[] = [
  {
    ...
    subsections: [
      ...
      },
      {
        ...
        content: 'The OMA-AI ecosystem consists of three main components: the Marketplace (discovery), the x402 Protocol (payments), and the SDK (integration). Together, they form a cohesive environment where machines can interact with digital services without human intervention.',
      },
      {
        id: 'quick-start',
        title: 'Quick Start',
    ...
    subsections: [
      {
        ...
        content: 'x402 is a payment protocol built on Base that enables autonomous agents to pay for API usage autonomously. It uses smart contracts for trustless transactions and automatic billing. By utilizing Layer 2 scaling, we ensure that micropayments are feasible even for high-volume agent interactions.',
      },
      {
        id: 'wallet-adapter',
        title: 'Wallet Adapter Setup',
        content: 'Configure your agent\'s wallet to make x402 payments. This requires a funded wallet on the Base network with USDC.',
    ...
    subsections: [
      {
        id: 'what-is-mcp',
        title: 'What is MCP?',
        content: 'Model Context Protocol (MCP) is an open standard that enables AI models to securely connect to external data sources and tools. OMA-AI acts as a discovery layer for MCP servers, allowing agents to find and connect to tools dynamically.',
    ...
    subsections: [
      {
        ...
        content: 'Direct HTTP endpoints for integration if you are not using our official SDK:',
    ...
    subsections: [
      {
        ...
        content: 'Protect your API keys at all costs. Since OMA-AI uses cryptographic proofs for many operations, the security of your private keys is paramount.',
        warning: 'Never expose private keys or sensitive API keys in client-side code or public repositories.',
      }
    ],
  }
];
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-14 py-12 flex flex-col lg:flex-row gap-12">
        ...
        <div className="flex-1 max-w-4xl">
          {filteredSections.map((section) => {
            ...
            return (
              <div key={section.id} className="mb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="flex items-center gap-4 mb-8">
                  <div className="w-12 h-12 bg-memoria-bg-card border border-memoria-border-muted rounded-sm flex items-center justify-center">
                    <section.icon className="text-memoria-text-hero" size={24} />
                  </div>
                  <div>
                    <h2 className="text-3xl font-light text-memoria-text-hero font-display tracking-tight">{section.title}</h2>
                    <p className="text-sm text-memoria-text-meta">{section.description}</p>
                  </div>
                </div>
                  ...
                  {section.subsections.map((sub) => (
                    <div key={sub.id} id={sub.id} className="group">
                      <div className="flex items-center gap-2 mb-4">
                        <h3 className="text-xl font-normal text-memoria-text-hero group-hover:text-memoria-text-secondary transition-colors font-display">{sub.title}</h3>
                        <Link href={`#${sub.id}`} className="opacity-0 group-hover:opacity-100 transition-opacity" aria-label={`Jump to ${sub.title} section`}>
                          <ChevronRight size={14} className="text-memoria-text-meta" />
                        </Link>
                      </div>
                      ...
                      {sub.note && (
                        <div className="mt-6 p-4 bg-blue-500/5 border border-blue-500/20 rounded-sm flex gap-3">
                          ...
                          <p className="text-xs text-blue-200/80 leading-relaxed"><span className="font-bold text-blue-400">Note:</span> {sub.note}</p>
                        </div>
                      )}
                      ...
                      {sub.warning && (
                        <div className="mt-6 p-4 bg-amber-500/5 border border-amber-500/20 rounded-sm flex gap-3">
                          ...
                          <p className="text-xs text-amber-200/80 leading-relaxed"><span className="font-bold text-amber-400">Warning:</span> {sub.warning}</p>
                        </div>
                      )}

                      {sub.tip && (
                        <div className="mt-6 p-4 bg-green-500/5 border border-green-500/20 rounded-sm flex gap-3">
                          <Lightbulb size={18} className="text-green-500 shrink-0" />
                          <p className="text-xs text-green-200/80 leading-relaxed"><span className="font-bold text-green-400">Tip:</span> {sub.tip}</p>
                        </div>
                      )}
