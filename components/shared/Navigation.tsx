    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-[#050510]/90 backdrop-blur-xl border-b border-white/5">
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-3 text-white cursor-pointer group">
          ...
          <span className="font-orbitron font-bold text-lg tracking-tight">{title}</span>
        </Link>
        
        {showStatus && (
          <div className="flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full ${status === 'ok' ? 'bg-green-500 animate-pulse shadow-lg shadow-green-500/50' : 'bg-red-500'}`} />
            <span className="text-sm text-green-400 font-medium">{status === 'ok' ? 'LIVE' : 'DOWN'}</span>
          </div>
        )}
      </div>
    </nav>
