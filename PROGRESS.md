...
## Ralph Iteration (2026-05-08): globals.css Cleanup

### Changes Made
1. **Removed duplicate `:root` block** — The legacy `:root` block (lines 95-128) was duplicating CSS vars already defined in `@theme`. Removed entirely.

2. **Replaced inline hardcoded hex colors with semantic variables:**
   - `background: #0c0c0c` → `var(--color-surface, #0c0c0c)` (.venice-card)
   - `border: 1px solid #1e1e1e` → `var(--color-border, #1e1e1e)` (.venice-card)
   - `border-color: #2e2e2e` → `var(--color-border, #2e2e2e)` (.venice-card:hover)
   - `color: #14f195` → `var(--color-success, #14f195)` (.x402-badge)
   - `color: #a1a1aa` → `var(--color-text-secondary, #a1a1aa)` (.price-pill)
   - `background: #12121f` → `var(--color-bg, #12121f)` (::-webkit-scrollbar-track)

3. **Added Firefox scrollbar support** — Added `scrollbar-width: thin` and `scrollbar-color` on `*` selector.

4. **Added `prefers-reduced-motion`** — Media query at top to disable animations for users who prefer reduced motion.
