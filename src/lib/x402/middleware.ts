...
import { create402Response, createPaymentRequirement, encodePaymentRequirement, verifyFacilitatorPayment, verifyBasePayment } from './server';
...
export async function verifyPayment(
  ...
  if (proof.type === 'facilitator' && proof.paymentId) {
    // Poll Coinbase CDP facilitator for payment confirmation
    const maxAttempts = 5;
    ...
    return { valid: false, error: `Payment not confirmed after ${maxAttempts} attempts` };
  }
  ...
  if (proof.type === 'onchain' && proof.txHash) {
    ...
    const result = await verifyBasePayment(
      ...
      expectedAmount || 0,
      PAYMENT_WALLET,
      PAYMENT_NETWORK === 'base' ? 'base' : 'base-sepolia'
    );
    
    if (!result.valid) {
      return { valid: false, error: result.error || 'On-chain verification failed' };
    }
