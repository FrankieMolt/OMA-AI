...
import { isOWSConfigured, signEVMPaymentWithOWS } from './ows-evm';
 ...
 *
 * Uses OWS (Open Wallet Standard) if available and configured, otherwise falls back to ethers.
 * OWS provides policy-gated, hardware-rooted signing with in-process memory protection.
 */
export async function signEVMPayment(params: {
  ...
  // Primary: OWS signing (local, policy-gated, keys encrypted at rest)
  if (isOWSConfigured()) {
    try {
      const owsSignature = await signEVMPaymentWithOWS({
        fromAddress,
        payTo,
        amountBigInt,
        validAfter,
      ...
      console.log('[x402] Signed with OWS (policy-gated)');
      return owsSignature;
    } catch (owsErr) {
      console.warn('[x402] OWS signing failed, falling back to ethers:', owsErr);
    }
  }

  // Fallback: ethers signing (requires X402_SIGNER_PRIVATE_KEY)
