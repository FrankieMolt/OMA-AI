...
export async function connectWallet(): Promise<{
  ...
  return { address: accounts[0], network, signedMessage };
}

/**
 * Verify signed message using viem
 */
export async function verifySignature(
  ...
  try {
    ...
    const recovered = await recoverMessageAddress({ message, signature: signedMessage as `0x${string}` });
...
export async function getWalletInfo() {
  ...
  return { address: accounts[0], chainId: parseInt(chainId, 16), isConnected: true };
}
...
export function onAccountChange(callback: (accounts: string[]) => void) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const ethereum = (window as any).ethereum;
  if (ethereum) ethereum.on('accountsChanged', callback);
}
...
export function onChainChange(callback: (chainId: string) => void) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const ethereum = (window as any).ethereum;
  if (ethereum) ethereum.on('chainChanged', callback);
}
