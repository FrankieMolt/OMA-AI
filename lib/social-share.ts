...
export async function shareFacebook(
  ...
  try {
    ...
    window.open(
      `https://www.facebook.com/sharer/sharer/sharer.php?u=${encodeURIComponent(url)}&title=${encodeURIComponent(title)}&description=${encodeURIComponent(description)}`,
      "_blank",
    );
  } catch (error) {
...
// LinkedIn Share
export async function shareLinkedIn(
  url: string,
  title: string,
  description: string,
): Promise<void> {
  try {
    ...
    window.open(
      `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(url)}&title=${encodeURIComponent(title)}&summary=${encodeURIComponent(description.substring(0, 200))}`,
      "_blank",
    );
  } catch (error) {
...
// Pinterest Share
export async function sharePinterest(
  url: string,
  description: string,
  image: string,
): Promise<void> {
