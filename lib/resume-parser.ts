...
export class ResumeParser {
  ...
  static extractExperience(text: string): WorkExperience[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        /\b(experience|work history|employment|career)\b/i.test(lineLower) &&
        line.length < 50
      ) {
      ...
      if (
        inExperienceSection &&
        /\b(education|skills|projects|certifications)\b/i.test(lineLower) &&
        line.length < 50
      ) {
      ...
      if (inExperienceSection) {
        ...
        const dateMatch = line.match(
          /((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s*\d{4}|\d{1,2}\/\d{4}|\d{4})\s*[-–]\s*((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s*\d{4}|\d{1,2}\/\d{4}|\d{4}|present|current)/i,
        );
  ...
  static extractEducation(text: string): Education[] {
    ...
    for (const line of lines) {
      ...
      if (
        /\b(education|academic|qualifications|degrees)\b/i.test(lineLower) &&
      ...
      if (
        inEducationSection &&
        /\b(experience|skills|projects|certifications)\b/i.test(lineLower) &&
        trimmedLine.length < 50
      ) {
      ...
      if (inEducationSection && trimmedLine.length > 3) {
        ...
        const degreeMatch = trimmedLine.match(
          /\b(Bachelor|Master|PhD|Doctorate|B\.S\.|M\.S\.|B\.A\.|M\.A\.|MBA|MD|JD)\b/i,
        );
        if (degreeMatch) {
          if (currentEdu.institution) {
            education.push(currentEdu as Education);
        ...
        const uniMatch = trimmedLine.match(
  ...
  static extractSummary(text: string): string {
    ...
    for (const line of lines) {
      ...
      if (
        /\b(summary|objective|profile|about me)\b/i.test(lineLower) &&
        trimmedLine.length < 50
      ) {
        inSummary = true;
        continue;
      ...
      if (
        inSummary &&
        (/\b(experience|education|skills|projects)\b/i.test(lineLower) ||
          trimmedLine === "")
      ) {
        break;
      }
