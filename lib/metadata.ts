...
export class MetadataGenerator {
   ...
   */
  static generateOpenGraphTags(config: SocialShareConfig): Record<string, string> {
    return {
   ...
   */
  static generateTwitterCardTags(config: SocialShareConfig): Record<string, string> {
    return {
  ...
  static generateProductTags(product: {
    ...
    return {
      ...
      'product:retailer_item_id': product.name.replace(/\s+/g, '-').toLowerCase(),
      'product:retailer_title': product.name,
      'product:is_digital': 'true',
      'product:in_stock': product.availability === 'in_stock' ? 'true' : 'false',
    };
  }
   ...
   */
  static generateStructuredData(type: 'website' | 'organization' | 'article', data: any): string {
    const base = {
