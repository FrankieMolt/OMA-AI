...
export class AIScorer {
   ...
   */
  async scoreResume(resume: ResumeData, jobDescription: string): Promise<ScoreResult> {
...
${description}`;
   ...
   */
  async batchScore(resumes: ResumeData[], jobDescription: string): Promise<Array<ScoreResult & { candidateName: string }>> {
   ...
   */
  private buildScoringPrompt(resume: ResumeData, jobDescription: string): string {
...
${resume.rawText.substring(0, 3000)}`;
  ...
  private parseScoringResponse(text: string): ScoreResult {
    try {
      ...
      if (jsonMatch) {
        ...
        return {
          ...
          skillMatch: parsed.skillMatch || { score: 0, matching: [], missing: [] },
          experienceMatch: parsed.experienceMatch || { score: 0, yearsMatched: false, relevanceScore: 0 },
          educationMatch: parsed.educationMatch || { score: 0, meetsRequirements: false },
    ...
    return this.fallbackScoring({ name: '', email: '', skills: [], experience: [], education: [], rawText: '' }, '');
  }

  /**
   * Fallback scoring when AI is unavailable
   */
  private fallbackScoring(resume: ResumeData, jobDescription: string): ScoreResult {
    ...
    const matchingSkills = jobSkills.filter(skill => 
      resumeLower.includes(skill) || resume.skills.some(s => s.toLowerCase().includes(skill))
    );
  ...
  private fallbackJobAnalysis(description: string): JobRequirements {
    ...
    const foundSkills = commonSkills.filter(skill => descLower.includes(skill));
    
    // Extract years of experience
    const yearsMatch = description.match(/(\d+)\+?\s*(?:years?|yrs?).*?(?:experience|exp)/i);
