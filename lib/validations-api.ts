...
export const paymentRequestSchema = z.object({
  ...
  paymentMethod: z.enum(['usdc_base', 'usdc_solana', 'eth'], 'Invalid payment method')
});
...
export const serviceCreateSchema = z.object({
  ...
  type: z.enum(['ai', 'mcp', 'data', 'blockchain', 'infrastructure'], 'Invalid service type'),
...
export const bountyCreateSchema = z.object({
  ...
  category: z.enum(['development', 'design', 'testing', 'documentation', 'other'], 'Invalid category'),
...
export const mcpServerSchema = z.object({
  ...
  description: z.string().min(10, 'Description must be at least 10 characters'),
  type: z.enum(['github', 'brave-search', 'filesystem', 'postgres', 'custom'], 'Invalid MCP server type'),
  endpoint: z.string().url('Invalid endpoint URL'),
  capabilities: z.array(z.string()).min(1, 'At least one capability is required'),
