...
export function getDemoData(endpoint: string) {
  ...
  if (path.includes("/api/services")) {
    return {
      services: DEFAULT_DEMO_DATA.services,
      stats: DEFAULT_DEMO_DATA.stats,
    };
  }
  ...
  if (path.includes("/api/bounties")) {
    return {
      tasks: DEFAULT_DEMO_DATA.tasks.filter((t) => t.status === "open"),
      stats: DEFAULT_DEMO_DATA.stats,
    };
  }
  ...
  if (path.includes("/api/marketplace") || path.includes("/api/links")) {
    return {
      services: DEFAULT_DEMO_DATA.services.slice(0, 5),
